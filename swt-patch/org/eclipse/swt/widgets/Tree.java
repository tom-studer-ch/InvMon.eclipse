 ...
 *
 * InvMon patch — based on SWT 3.132.0 (Eclipse 2025-12)
 * Fixes Windows dark mode issues in Tree widget:
 *   - Header grid lines use configurable color (display.tableHeaderLinePixel)
 *     instead of hardcoded OS.COLOR_3DFACE
 *   - Column divider lines in content area use configurable color
 *   - Horizontal row divider lines suppressed when custom color is set
 *   - setLinesVisible(true) enables customDraw regardless of hwndParent
 *******************************************************************************/
...
LRESULT CDDS_ITEMPOSTPAINT (NMTVCUSTOMDRAW nmcd, long wParam, long lParam) {
	...
	*/
	...
	if (linesVisible && display.tableHeaderLinePixel == -1) {
		if ((style & SWT.FULL_SELECTION) != 0) {
			if (columnCount != 0) {
				HDITEM hdItem = new HDITEM ();
				hdItem.mask = OS.HDI_WIDTH;
...
LRESULT CDDS_POSTPAINT (NMTVCUSTOMDRAW nmcd, long wParam, long lParam) {
	...
	if (OS.IsWindowVisible (handle)) {
		...
		if (linesVisible) {
			long hDC = nmcd.hdc;
			boolean useCustomGridColor = display.tableHeaderLinePixel != -1;
			long gridPen = 0, oldGridPen = 0;
			if (useCustomGridColor) {
				gridPen = OS.CreatePen (OS.PS_SOLID, 1, display.tableHeaderLinePixel);
				oldGridPen = OS.SelectObject (hDC, gridPen);
			}
			if (hwndHeader != 0) {
				int x = 0;
				RECT rect = new RECT ();
				...
				for (int i=0; i<columnCount; i++) {
					...
					if (useCustomGridColor) {
						OS.Polyline(hDC, new int[] {rect.right - 1, rect.top, rect.right - 1, rect.bottom}, 2);
					} else {
			...
			*/
			...
			if (useCustomGridColor) {
				OS.SelectObject (hDC, oldGridPen);
				OS.DeleteObject (gridPen);
			}
		}
	}
	return new LRESULT (OS.CDRF_DODEFAULT);
...
public void setLinesVisible (boolean show) {
	...
	linesVisible = show;
	if (linesVisible) customDraw = true;
	OS.InvalidateRect (handle, null, true);
	if (hwndHeader != 0) OS.InvalidateRect (hwndHeader, null, true);
}
