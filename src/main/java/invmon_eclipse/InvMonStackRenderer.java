...
public class InvMonStackRenderer extends LazyStackRenderer {
	...
	void subscribeTopicActivateChanged(@UIEventTopic(UIEvents.UILifeCycle.ACTIVATE) Event event) {
		...
		List<MUIElement> activeElements = modelService.findElements(modelService.getTopLevelWindowFor(newActivePart), null, MUIElement.class,
	...
	@Inject
	@Optional
	void subscribeTopicSelectedelementChanged(@UIEventTopic(UIEvents.ElementContainer.TOPIC_SELECTEDELEMENT) Event event) {
		if (tabStateHandler == null) {
			tabStateHandler = new TabStateHandler();
		}
		tabStateHandler.handleEvent(event);
	...
	public Object createWidget(MUIElement element, Object parent) {
		...
		if (!element.getTags().contains("Standalone")) {
			
			// Standard StackRenderer Code
			...
			// "Standalone" -- simple, not multiple tabs, stylable via class "Standalone"
			
			final CTabFolder tabFolder = new CTabFolder(parentComposite, SWT.NONE);
			tabFolder.setData("org.eclipse.e4.ui.css.CssClassName", "Standalone");
			bindWidget(element, tabFolder); // ?? Do we need this ?
			addTopRight(tabFolder);
			return tabFolder;
		}
	...
	protected void showTab(MUIElement element) {
		...
		if (!(getParentWidget(element) instanceof CTabFolder)) {			
			Composite standaloneStack = (Composite) getParentWidget(element);
			Control control = (Control) element.getWidget();

			if (control == null) {
			control = (Control) renderer.createGui(element);
			}
