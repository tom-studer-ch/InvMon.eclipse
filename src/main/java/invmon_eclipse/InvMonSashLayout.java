...
*
* Based on: Eclipse 4.29 (2023-09) - org.eclipse.e4.ui.workbench.renderers.swt.SashLayout
...
public class InvMonSashLayout extends Layout {
	...
	private void onMouseMove(MouseEvent e) {
		if (!draggingSashes) {
			...
			try {
				...
				// SWT Win requires a synchronous layout call to update the UI
	...
	protected List<SashRect> getSashRects(int x, int y) {
		...
		for (SashRect sr : sashes) {
			if (sr.rect.intersects(target) && !sr.left.getTags().contains(IPresentationEngine.NO_MOVE) && !sr.container.getTags().contains(IPresentationEngine.NO_MOVE))
				srs.add(sr);
		}
		return srs;
	}
	...
	private void tileSubNodes(Rectangle bounds, MUIElement node) {
		...
		for (MUIElement subNode : visibleChildren) {
			...
			Rectangle subBounds = sashContainer.isHorizontal() ? new Rectangle(
					...
					bounds.x + marginLeft, tilePos + marginTop, bounds.width - marginLeft - marginRight, newSize - marginTop - marginBottom);
			tilePos += newSize;
